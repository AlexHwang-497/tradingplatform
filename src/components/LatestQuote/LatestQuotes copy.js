import { useEffect, useState } from "react";

import Card from '../UI/Card'
import classes from './LatestQuote.module.css'
import apisauce from 'apisauce'
import config from "./config";


const LatestQuotes = () =>{
    const api = apisauce.create({
        baseURL:config.APCA_API_BASE_URL,
        headers:{
            'APCA-API-KEY-ID':config.APCA_API_KEY_ID,
            'APCA-API-SECRET-KEY':config.APCA_API_SECRET_KEY
        },
        timeout:5000

    })
    console.log('api Latest:' + JSON.stringify(api))

    const getAccount = () => {api.get('v2/account')}
    console.log('getAccount Latest:' + JSON.stringify(getAccount))

    useEffect(  ()=>{
        // const fetchLatestQuote = async() =>{
        //     const response = await fetch('https://paper-api.alpaca.markets/v2/stocks/AAPL/quotes/latest', {headers:{
        //         'APCA-API-KEY-ID':'PKYV5PSEAEZPXTN6MZH0',
        //         'APCA-API-SECRET-KEY':'D8CzWfb7Y55cPhOHVJfBhRYqbMVtucfGOOCURueS',
        //         'Content-Type':'application/json'
        //     }})
        //     if(!response.ok){
        //         throw new Error('Something went wrong!!!')
        //     }
        //     console.log('response Latest:'+JSON.stringify(response))
        //     const responseData = await response.json()
        //     const loadQuote = []

        // }
        //  fetchLatestQuote()


        var myHeaders = new Headers();
        myHeaders.append("APCA-API-KEY-ID", "PKYV5PSEAEZPXTN6MZH0");
        myHeaders.append("APCA-API-SECRET-KEY", "D8CzWfb7Y55cPhOHVJfBhRYqbMVtucfGOOCURueS");
        
        var raw = "";
        
        var requestOptions = {
          method: 'GET',
          headers: myHeaders,
          body: raw,
          redirect: 'follow'
        };
        
        fetch("https://data.alpaca.markets/v2/stocks/AAPL/trades/latest", requestOptions)
          .then(response => response.text())
          .then(result => console.log(result))
          .catch(error => console.log('error', error));

    },[])

    
    // d55fa92649a73345ab992bcd3b776acd    --client id 
    //ece54bc871926a061f0460ca6522fd24c29f78a9 -- client secret



    return(
        <Card>
            <div>Here are the LatestQuote</div>
        </Card>
    )
}

export default LatestQuotes